   ...
   correctionPhaseNS(6.0)
{
   ...
   FillHO = iConfig.getUntrackedParameter<bool>("FillHO", true);
   //TotalChargeThreshold = iConfig.getUntrackedParameter<double>("TotalChargeThreshold", 10);
   //TotalChargeThreshold = iConfig.getUntrackedParameter<double>("TotalChargeThreshold", -9999);
   TotalChargeThreshold = iConfig.getUntrackedParameter<double>("TotalChargeThreshold");

   sHBHERecHitCollection = iConfig.getUntrackedParameter<string>("HBHERecHits", "hbhereco");
 
   IsCosmic_ = iConfig.getUntrackedParameter<bool>("IsCosmic");
   Initialize();
}
//---------------------------------------------------------------------------
...
void HcalNoiseAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   Handle<TrackCollection> hCosmicMuon;
   iEvent.getByLabel("cosmicMuons", hCosmicMuon);
   ...
   for(HBHEDigiCollection::const_iterator iter = hHBHEDigis->begin(); iter != hHBHEDigis->end(); iter++)
   {
      ...
      Hit_RBXIndex[PulseCount] = RBXIndex;
...
void HcalNoiseAnalyzer::beginJob()
{
   ...
   if(FillHBHE == true)
   {
      ...
      OutputTree->Branch("Charge", &Charge, "Charge[PulseCount][10]/D");
      OutputTree->Branch("Pedestal", &Pedestal, "Pedestal[PulseCount][10]/D");
      OutputTree->Branch("Energy", &Energy, "Energy[PulseCount]/D");
      OutputTree->Branch("IEta", &IEta, "IEta[PulseCount]/I");
      OutputTree->Branch("IPhi", &IPhi, "IPhi[PulseCount]/I");
      OutputTree->Branch("Depth", &Depth, "Depth[PulseCount]/I");
      OutputTree->Branch("Hit_RBXIndex", &Hit_RBXIndex, "Hit_RBXIndex[PulseCount]/I");
      OutputTree->Branch("RecHitTime", &RecHitTime, "RecHitTime[PulseCount]/D");
      OutputTree->Branch("FlagWord", &FlagWord, "FlagWord[PulseCount]/i");
      OutputTree->Branch("AuxWord", &AuxWord, "AuxWord[PulseCount]/i");
      OutputTree->Branch("RespCorrGain", &RespCorrGain, "RespCorrGain[PulseCount]/D");
      OutputTree->Branch("fCorr", &FCorr, "fCorr[PulseCount]/D");
      OutputTree->Branch("SamplesToAdd", &SamplesToAdd, "SamplesToAdd[PulseCount]/D");
   }
   ...
   if(FillHO == true)
   {
      ...
      OutputTree->Branch("HOADC", &HOADC, "HOADC[HOPulseCount][10]/I");
      OutputTree->Branch("HOCharge", &HOCharge, "HOCharge[HOPulseCount][10]/D");
      OutputTree->Branch("HOPedestal", &HOPedestal, "HOPedestal[HOPulseCount][10]/D");
      OutputTree->Branch("HOIPhi", &HOIPhi, "HOIPhi[HOPulseCount]/I");
      OutputTree->Branch("HOIEta", &HOIEta, "HOIEta[HOPulseCount]/I");
      OutputTree->Branch("HOEnergy", &HOEnergy, "HOEnergy[HOPulseCount]/D");
   }
